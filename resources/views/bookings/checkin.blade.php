...
@section('title', 'Bookings (Check In)')
    ...
    <div class="content-header row my-1">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-start mb-0">Bookings (Check In)</h2>
                    <div class="breadcrumb-wrapper">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">
                                <a href="javascript:void(0);">Bookings (Check In)</a>
                            </li>
                        </ol>
                    </div>
                </div>
    ...
    <div class="content-body">
        ...
        <div class="card">
            <div class="card-header">
                ...
                @if (!is_null($checkinList) && !empty($checkinList))
                    {{ $checkinList->onEachSide(2)->links('layout.pagination') }}
                @endif
                    ...
                    <table class="table table-hover table-hover-animation text-center">
                        ...
                        <tbody>
                            <form action="{{ route('bookings.destroy', ['booking' => 'empty']) }}"
                                ...
                                @csrf
                                @method('DELETE')
                                @forelse ($checkinList as $booking)
                                    <tr>
                                        ...
                                        <td style="vertical-align: middle;">
                                            <a class="btn btn-relief-outline-primary"
                                                href="{{ route('bookings.checkin.store', ['booking' => $booking->id]) }}"
                                                id="check_in_{{ $booking->id }}" class="check_in_{{ $booking->id }}">
                                                <span class="spinner-grow spinner-grow-sm visually-hidden"
                                                    id="check_in_{{ $booking->id }}_loading" role="status"
                                                    aria-hidden="true"></span>
                                                <span class="ms-25 align-middle"
                                                    id="check_in_{{ $booking->id }}_label">Check In</span>
                                                <span class="ms-25 align-middle visually-hidden"
                                                    id="check_in_{{ $booking->id }}_labelLoading">Loading...</span>
                                            </a>
                                        </td>
                                    </tr>
                                @empty
