    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1">Remove Cabin - Needs Cleaning</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"
                    id="basicModal-close"></button>
            </div>
            <div class="modal-body mb-0 pb-0">
                <form action="{{ route('cabins.needs-cleaning.store') }}" method="POST" id="form_needs_cleaning_cabin"
                    enctype="multipart/form-data">

                    @csrf
                    @method('DELETE')
                    @include('cabins.needs-cleaning.modal.form-fields')
                </form>
...
<script>
    $(function() {
        ...
        $('#btn-submit').on('click', function() {
            $('#form_needs_cleaning_cabin').submit();
        });
    });
</script>
